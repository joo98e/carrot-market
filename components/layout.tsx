...
const CanGoBack = ({ move }: ICanGoBack) => {
  return <button className="self-start">&larr;</button>
}

const Layout = ({ title, canGoBack, hasTabBar, children }: LayoutProps) => {
  const router = useRouter()
  const prevMove = () => router.back()
  ...
  return (
    <div className="max-w-lg">
      ...
      <div className={makeClassName('pt16', hasTabBar ? 'pb-16' : '')}>
        {children}
      </div>
      {hasTabBar && (
        <nav className="bg-white w-full max-w-lg text-gray-800 border-t fixed bottom-0 pb-6 pt-3 flex justify-between items-center">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="mt-1">동네 생활</span>
          </div>
          <div className="flex flex-col items-center">
            <svg
              className="w-6 h-6"
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="mt-1">라이브 스트리밍</span>
          </div>
          <div className="flex flex-col items-center">
            <svg
              className="w-6 h-6"
